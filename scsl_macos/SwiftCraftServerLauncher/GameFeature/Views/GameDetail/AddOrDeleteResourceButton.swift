...
struct AddOrDeleteResourceButton: View {
    ...
    @State private var preloadedCompatibleServers: [ServerInstance] = []
            ...
            .sheet(
                ...
                content: {
                    GlobalResourceSheet(
                        ...
                        preloadedCompatibleServers: preloadedCompatibleServers
                    )
    ...
    private func deleteFile(fileName: String?, isUpdate: Bool = false) {
        // 检查 query 是否是有效的资源类型
        let validResourceTypes = ["mod", "plugin", "datapack", "shader", "resourcepack"]
        ...
        if case .game = selectedItem {
            ...
            case .idle:
                ...
                if type {
                    ...
                    let queryLowercased = query.lowercased()
                    if queryLowercased == "plugin" {
                        let hasPaperServer = serverRepository.servers.contains {
                            $0.serverType == .paper
                        }
                        if !hasPaperServer {
                            activeAlert = .noServer
                            return
                        }
                    } else if queryLowercased == "mod" {
                        let hasModServer = serverRepository.servers.contains {
                            $0.serverType == .fabric || $0.serverType == .forge
                        }
                        if !hasModServer {
                            activeAlert = .noServer
                            return
    ...
    private func loadProjectDetailBeforeOpeningSheet() async {
        ...
        await MainActor.run {
            ...
            preloadedCompatibleServers = result.compatibleServers
